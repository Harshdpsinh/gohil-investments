...
export default function Modal({ open, onClose, title, children, size = 'md', subtitle = '', footerContent = null }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden p-4">
      ...
      <div className={`animate-scaleIn relative flex max-h-[90vh] w-full min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200/10 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.28)] dark:bg-gradient-to-br dark:from-slate-900 dark:to-[#101827] dark:shadow-[0_24px_80px_rgba(0,0,0,0.62),0_0_0_1px_rgba(255,255,255,0.04)] ${widths[size]} max-w-[calc(100vw-2rem)]`}>
        ...
        <div className="min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-6 py-5 text-slate-800 dark:text-slate-200">
          {children}
        </div>

        {footerContent && (
          <div className="flex flex-shrink-0 justify-end gap-3 border-t border-slate-200/80 bg-slate-50/80 px-6 py-4 dark:border-slate-400/10 dark:bg-slate-950/30">
            {footerContent}
          </div>
        )}
      </div>
